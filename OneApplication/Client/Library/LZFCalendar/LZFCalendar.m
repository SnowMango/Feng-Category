...
const NSCalendarUnit CalendarUnit = NSCalendarUnitYear | NSCalendarUnitMonth |NSCalendarUnitDay |NSCalendarUnitWeekday;

@implementation LZFCalendar

+ (NSDate *)SolarFromInt:(long)g
{
...
+ (Lunar *)solarToLunar:(NSDate *)solar{
    ...
    NSCalendar * ca = [NSCalendar calendarWithIdentifier:NSCalendarIdentifierGregorian];
    NSDateComponents *components = [ca components:CalendarUnit fromDate:solar];
    wCurYear = (int)components.year;
    wCurMonth = (int)components.month;
    wCurDay = (int)components.day;
    
    Lunar *lunar = [[Lunar alloc] init];
    int index = wCurYear - solar_1_1[0];
    int data = (wCurYear << 9) | (wCurMonth << 5) | (wCurDay);
    int solar11 = 0;
    ...
    int m = GetBitInt(solar11, 4, 5);
    int d = GetBitInt(solar11, 5, 0);
    long offset = SolarToInt(wCurYear, wCurMonth, wCurDay) - SolarToInt(y, m, d);
...
{
    //农历月份名
    NSArray *cMonName =  @[@"*",@"正",@"二",@"三",@"四",@"五",@"六",@"七",@"八",@"九",@"十",@"十一",@"腊"];
    return cMonName;
}
...
@implementation NSDate (LZFCalendar)

- (NSDate *)firstDayOfMonth
{
    
    NSCalendar *ca = [NSCalendar calendarWithIdentifier:NSCalendarIdentifierGregorian];
    NSDateComponents *componentsCurrentDate = [ca components:CalendarUnit fromDate:self];
    NSDateComponents *componentsNewDate = [[NSDateComponents alloc] init];
    componentsNewDate.year = componentsCurrentDate.year;
    componentsNewDate.month = componentsCurrentDate.month;
    componentsNewDate.weekOfMonth = 1;
    componentsNewDate.day = 1;
    return [ca dateFromComponents:componentsNewDate];
}
...
- (NSDate *)addMonths:(NSInteger)months
{
    ...
    return [ca dateByAddingComponents:components toDate:self options:0];
}
...
- (NSUInteger)numberOfWeeks
{
    ...
    NSDateComponents *componentsA = [ca components:NSCalendarUnitWeekOfYear fromDate:firstDay];
    NSDateComponents *componentsB = [ca components:NSCalendarUnitWeekOfYear fromDate:lastDay];
...
{
    NSCalendar *ca = [NSCalendar calendarWithIdentifier:NSCalendarIdentifierGregorian];
    NSDateComponents *componentsCurrentDate = [ca components:NSCalendarUnitYear|NSCalendarUnitMonth|NSCalendarUnitDay|NSCalendarUnitWeekday|NSCalendarUnitWeekOfMonth fromDate:self];
...
{
    NSCalendar *ca =[NSCalendar calendarWithIdentifier:NSCalendarIdentifierGregorian];
    NSDateComponents *com = [ca components:NSCalendarUnitMonth fromDate:self];
    return com.month;
}
